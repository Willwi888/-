...
const VideoPlayer: React.FC<VideoPlayerProps> = ({ timedLyrics, audioUrl, imageUrl, songTitle, artistName, onBack }) => {
  ...
  const currentIndex = useMemo(() => {
    ...
    // Otherwise, before the song starts, point to the second dummy lyric (so first real lyric is at pos 0)
    return 1;

  }, [currentTime, timedLyrics]);

  const lyricsContainerShift = useMemo(() => {
    if (!timedLyrics.length) return 0;
    
    const baseShift = (currentIndex - 2) * fontSize * 1.5;
    const currentLyric = timedLyrics[currentIndex - 2];
    
    if (currentLyric && currentTime >= currentLyric.startTime && currentTime < currentLyric.endTime) {
      const progress = (currentTime - currentLyric.startTime) / (currentLyric.endTime - currentLyric.startTime);
      // Create a subtle "settle in" animation at the beginning of each line
      if (progress < 0.25) {
        const easeOutQuart = 1 - Math.pow(1 - (progress / 0.25), 4);
        return baseShift + (fontSize * 1.5 * (1 - easeOutQuart));
      }
    }
    return baseShift;
  }, [currentIndex, currentTime, timedLyrics, fontSize]);

  const currentTheme = colorThemes[colorTheme];

  const getLyricStyle = useCallback((position: -2 | -1 | 0 | 1 | 2): React.CSSProperties => {
    ...
    switch (position) {
      case 0:
        return {
          ...
          // Color is handled by KaraokeLyric component
        };
      case -1:
        return {
          ...baseStyle,
      ...
      case -2:
        return {
          ...baseStyle,
          opacity: 0.5,
          transform: 'translate(-50%, -340%) scale(0.8)',
          color: currentTheme.inactive2,
        };
      case 2:
        return {
          ...baseStyle,
          opacity: 0.5,
          transform: 'translate(-50%, 240%) scale(0.8)',
          color: currentTheme.inactive2,
        };
      default:
        return { display: 'none' };
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < totalFrames; i++) {
        ...
        const currentIdx = timedLyrics.findIndex(l => t >= l.startTime && t < l.endTime);
        const activeLyricIndexInRenderList = (currentIdx !== -1) ? currentIdx + 2 
            : (timedLyrics.length > 0 && t >= timedLyrics[timedLyrics.length - 1].endTime) ? timedLyrics.length + 2
            : 1;
        ...
        for (let j = -2; j <= 2; j++) {
            const index = activeLyricIndexInRenderList + j;
            if (index < 0 || index >= lyricsToRender.length) continue;
            
            const lyric = lyricsToRender[index];
            const style = getLyricStyle(j as any);

            ctx.save();
            ctx.globalAlpha = style.opacity as number;
            const scale = parseFloat((style.transform as string).match(/scale\(([^)]+)\)/)?.[1] || '1');
            ctx.font = `${fontWeight} ${renderFontSize * scale}px ${fontFamily}`;
            
            const yOffsetMatch = (style.transform as string).match(/translate\(-50%, ([^%]+)%\)/);
            const yOffsetPercent = parseFloat(yOffsetMatch ? yOffsetMatch[1] : '0');
            const yPos = canvas.height / 2 + (yOffsetPercent / 100 * renderFontSize * 1.5);
            ...
            if (j === 0) { // Active karaoke line
                const progress = (t - lyric.startTime) / (lyric.endTime - lyric.startTime);
                ...
                ctx.save();
                ctx.beginPath();
                ctx.rect(xPos - textWidth / 2, yPos - renderFontSize, textWidth * progress, renderFontSize * 2);
                ctx.clip();
        ...
        // Fix: canvas.toBlob is callback-based and was being used as a Promise. It has been promisified.
  ...
  }, [timedLyrics, audioUrl, imageUrl, songTitle, artistName, fontSize, fontFamily, fontWeight, strokeColor, strokeWidth, colorTheme, resolution, includeAlbumArt, albumArtSize, albumArtPosition, lyricsToRender, getLyricStyle, currentTheme]);
          ...
          <div 
            ...
            >
              <div
                  ...
                  style={{ transform: `translateY(${-lyricsContainerShift}px)` }}
              >
                  {[-2, -1, 0, 1, 2].map(position => {
                      ...
                      const style = {
                        ...getLyricStyle(position as any),
                        fontSize: `${fontSize * parseFloat((getLyricStyle(position as any).transform as string).match(/scale\(([^)]+)\)/)?.[1] || '1')}px`,
                        lineHeight: 1.4,
                      };
                      ...
                      return (
                          <p key={`lyric-${index}`} style={style} className="tracking-wide">
                              {lyric.text}
                          </p>
                      );
                  })}
